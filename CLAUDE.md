...
For icons, call `registerSiteIcons()` from `stackwright-generated/icons` (generated by `stackwright-prebuild`). This file tree-shakes `lucide-react` to only the icons used in the site's YAML. **Fallback**: `registerDefaultIcons()` from `@stackwright/icons` uses the curated ~43-icon preset when the generated file is not present. Run `pnpm predev` to (re)generate `stackwright-generated/icons.ts`.
