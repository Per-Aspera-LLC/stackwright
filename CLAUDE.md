...
**Grammar / JSON Schema Generation**: `@stackwright/types` is the single source of truth for the Stackwright grammar. It runs `typescript-json-schema` to produce `theme-schema.json`, `content-schema.json`, and `siteconfig-schema.json` — the machine-readable grammar specification used for IDE YAML validation. Must be regenerated after type changes (`pnpm generate-schemas`). The Zod migration (ROADMAP.md) will make the grammar introspectable at runtime as well.
