...
test.describe('Bundle Size Benchmarks', () => {
  ...
  test('first-load JS bundle size', async () => {
    ...
    console.log(`  Budget: ${Math.round(budget.max / 1024)}KB (warn at ${Math.round(budget.warn / 1024)}KB)`);
    console.log(`  Status: ${passed ? '✅ PASS' : '❌ FAIL'} ${warning ? '⚠️ WARNING' : ''}`);

    expect(stats.firstLoadJS, `First-load JS is ${sizeKB}KB, budget is ${Math.round(budget.max / 1024)}KB`).toBeLessThanOrEqual(budget.max);
    
    if (warning) {
      console.warn(`⚠️ Warning: First-load JS exceeds ${Math.round(budget.warn / 1024)}KB threshold`);
    }
  });
  ...
  test('total JS bundle size', async () => {
    ...
    expect(stats.allPagesJS, `Total JS is ${sizeKB}KB, budget is ${Math.round(budget.max / 1024)}KB`).toBeLessThanOrEqual(budget.max);
