...
test.describe('Build Time Benchmarks', () => {
  ...
  test('stackwright-prebuild performance', async () => {
    ...
    // Assert
    expect(duration, `Prebuild took ${duration}ms, budget is ${budget.max}ms`).toBeLessThanOrEqual(budget.max);
  ...
  test('next build performance', async () => {
    ...
    // Assert
    expect(duration, `Next build took ${duration}ms, budget is ${budget.max}ms`).toBeLessThanOrEqual(budget.max);
  ...
  test('combined build pipeline performance', async () => {
    ...
    expect(totalDuration, `Total build took ${totalDuration}ms, budget is ${totalBudget}ms`).toBeLessThanOrEqual(totalBudget);
  });
});
