...
describe('Carousel', () => {
  ...
  it('renders carousel items with titles and text', () => {
    render(<Carousel label="test-carousel" heading="Test Carousel" items={makeItems(3)} />);
  ...
  it('shows navigation buttons when items exceed visible slots', () => {
    render(<Carousel label="test-carousel" heading="Test Carousel" items={makeItems(6)} />);
  ...
  describe('ARIA attributes', () => {
    it('has role="region" and aria-roledescription="carousel"', () => {
      render(<Carousel label="test-carousel" heading="My Carousel" items={makeItems(3)} />);
      const container = screen.getByRole('region');
      expect(container).toHaveAttribute('aria-roledescription', 'carousel');
    });

    it('uses heading as aria-label', () => {
      render(<Carousel label="test-carousel" heading="Featured Items" items={makeItems(3)} />);
      const container = screen.getByRole('region');
      expect(container).toHaveAttribute('aria-label', 'Featured Items');
    });

    it('sets aria-live="polite" when autoPlay is off', () => {
      render(<Carousel label="test-carousel" heading="Test" items={makeItems(3)} />);
      const container = screen.getByRole('region');
    ...
    it('sets aria-live="off" when autoPlay is on', () => {
      render(
        <Carousel label="test-carousel" heading="Test" autoPlay={true} items={makeItems(3)} />
      );
      const container = screen.getByRole('region');
  ...
  describe('responsive behavior', () => {
    it('shows 1 item on extra small screens', () => {
      ...
      render(<Carousel label="test-carousel" heading="Test" items={makeItems(4)} />);
      const slides = screen.getAllByRole('group');
