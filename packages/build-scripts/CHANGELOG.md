...
- 1202590: `runPrebuild({ logSink })` now also sets `process.env.STACKWRIGHT_LOG_STREAM`
...
- 3dd0630: Add lucide-react export existence validator to `generateIconManifest()`.
...
- b6e3572: Add structured NDJSON prebuild telemetry sink (swp-6xr3.1, child of swp-6xr3).
...
- 0d2c3b8: Fix icon generator to handle kebab-case and lowercase YAML icon names per
...
- 2c1e586: **fix(build-scripts): dedup plugin-declared type warnings + thread prebuild.unknownContentTypes from yml**
