...
export function registerContentTypeTools(server: McpServer): void {
  ...
  const KNOWN_CONTENT_TYPES = [
    'main', 'carousel', 'timeline', 'icon_grid', 'tabbed_content',
    'code_block', 'feature_list', 'testimonial_grid', 'faq',
    'pricing_table', 'alert', 'contact_form_stub',
  ];
  ...
  server.tool(
    ...
    'Show a visual preview screenshot of a Stackwright content type. Returns a PNG image of how the component renders with example data.',
    {
      content_type: z.string().describe(
        `The content type to preview. One of: ${KNOWN_CONTENT_TYPES.join(', ')}`,
      ),
    },
    async ({ content_type }) => {
