...
describe('MCP Tools Integration', () => {
  ...
  describe('Page Tools', () => {
    ...
    it('list_pages shows existing pages', async () => {
      ...
      fs.writeFileSync(path.join(page1Dir, 'content.yml'), makePageYaml('about', 'About Us'));
    ...
    it('get_page returns page YAML content', async () => {
      ...
      fs.writeFileSync(path.join(pageDir, 'content.yml'), yamlContent);
    ...
    it('write_page updates existing page', async () => {
      ...
      fs.ensureDirSync(pageDir);
      fs.writeFileSync(path.join(pageDir, 'content.yml'), makePageYaml('existing', 'Old Title'));
    ...
    it('validate_pages returns success for valid pages', async () => {
      const pageDir = path.join(pagesDir, 'about');
      fs.ensureDirSync(pageDir);
      fs.writeFileSync(path.join(pageDir, 'content.yml'), makePageYaml('about', 'About Us'));
    ...
    it('validate_pages returns errors for invalid pages', async () => {
      ...
      fs.ensureDirSync(pageDir);
      fs.writeFileSync(path.join(pageDir, 'content.yml'), 'invalid: yaml: content: [[');
  ...
  describe('Project Tools', () => {
    ...
    it('get_project_info returns project information', async () => {
      ...
      fs.writeFileSync(path.join(testDir, 'package.json'), makePackageJson('test-project'));
  ...
  describe('Git Ops Tools', () => {
    ...
    it('stage_changes stages content files', async () => {
      ...
      fs.ensureDirSync(pageDir);
      fs.writeFileSync(path.join(pageDir, 'content.yml'), makePageYaml('test-page', 'Test Page'));
