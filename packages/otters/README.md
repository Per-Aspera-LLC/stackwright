...
| 🦦📚 **Collection Otter** | Structured content | Optional Phase 4 — invoked by Foreman when blog/docs/portfolio needed |
...
└───────┬────────────────┘
        ...
        ▼
┌────────────────────────┐
│  Collection Otter      │ ◄── Phase 4 (optional)
│  (blog/docs/portfolio) │
└───────┬────────────────┘
        │ creates content/<name>/ + listing page
        ▼
┌────────────────────────┐
...
The following reference sites demonstrate complete otter pipeline output. Each contains `BRAND_BRIEF.md`, `stackwright.yml`, and all page YAML files — exactly what Brand Otter → Theme Otter → Page Otter would produce. The portfolio and B2B SaaS examples also include collection content generated by the Collection Otter.
...
| [`examples/saas-example/`](../../examples/saas-example/) | B2B SaaS / analytics | 5 (Home, Features, Pricing, About, Contact) + blog collection | Indigo + amber energy |
...
└── src/
    ...
    ├── stackwright-collection-otter.json
